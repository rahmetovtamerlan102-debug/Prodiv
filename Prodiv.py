    ...
    try:
        ...
        async def message_handler(event):
            if event.message.from_id == entity.id:
                response_event.set()
                logger.info(f"Пойман текстовый ответ от {username}")
        ...
        async def callback_handler(event):
            if event.query.user_id == bot_id:
                response_event.set()
                logger.info(f"Поймана callback-кнопка от {username}")
        
        # Ловим редактирование сообщений (кнопки без текста)
        @client.on(events.MessageEdited(chats=entity))
        async def edit_handler(event):
            if event.message.from_id == entity.id:
                response_event.set()
                logger.info(f"Поймано редактирование сообщения от {username}")
        
        # Ловим сырые события (самый надёжный способ)
        @client.on(events.Raw)
        async def raw_handler(event):
            if hasattr(event, 'message') and event.message:
                if event.message.from_id == entity.id:
                    response_event.set()
                    logger.info(f"Поймано сырое событие от {username}")

        start = time.perf_counter()
        await client.send_message(entity, "/start")
        logger.info(f"Отправлен /start боту {username}")
