...
try:
    ...
    HAS_GIFTS = False
    print("Библиотека telegram_gift_fetcher не установлена. Подарки не будут отображаться.")
...
# Веса для кастомного рейтинга активности
WEIGHTS = {
    'message_count': 0.2,
    'reaction_count': 0.5,
    'avg_msg_length': 0.2,
...
# ========== БАЗА ДАННЫХ ДЛЯ ИСТОРИИ USERNAME ==========
            ...
            async for msg in client.iter_messages(chat_id, from_user=target_user.id, offset_date=cutoff_date, limit=MAX_MESSAGES_PER_CHAT):
                ...
                if msg.reactions:
                    for r in msg.reactions.results:
                        ...
                        # Детализация реакций (для расширенного отчёта)
...
def calculate_custom_rating(metrics):
    """Кастомный рейтинг активности (0-100)"""
    raw = 0
...
async def get_full_user_info(client, target):
    try:
        ...
        # Получение официального рейтинга Telegram (если доступен)
...
async def get_collectible_gifts(client, target_user):
    try:
        result = await client(GetCollectibleEmojiStatusesRequest())
        gifts = []
        if result.statuses:
            for s in result.statuses:
                gifts.append(str(s.document_id))
        return gifts
    except Exception as e:
        logger.error(f"Ошибка получения коллекционных подарков: {e}")
        return []

async def get_stories_count(client, target_user):
    try:
        stories = await client(GetStoriesRequest(peer=target_user.id, limit=1))
        if stories.count:
            return stories.count
        return 0
    except Exception as e:
        logger.error(f"Ошибка получения сторис: {e}")
        return 0

async def get_gifts_info(client, user_id):
    """Получение подарков (отправленных и полученных)"""
    if not HAS_GIFTS:
        return None, None
    try:
...
async def search_user(target_input):
    ...
    if getattr(user, 'premium', False) and hasattr(full_user, 'premium_expires'):
        report += f"▸ Premium до: {full_user.premium_expires.strftime('%Y-%m-%d')}\n"
    ...
    if official_rating is not None:
        ...
        report += f"▸ Официальный рейтинг Telegram: не определён\n"
