...
# ========== 9. ОТЧЁТ (HTML, без ошибок форматирования) ==========
def make_report(info: Dict[str, Any]) -> str:
    ...
    if info["user_id"]:
        lines.append(f"🆔 <b>ID (userinfobot):</b> <code>{info['user_id']}</code>")
...
async def cmd_start(message: types.Message):
    await message.reply(
        "🔮 <b>OSINT бот (правильная цепочка)</b>\n\n"
        "Отправь юзернейм.\n\n"
        "<b>Цепочка:</b>\n"
