...
from aiohttp import web  # ДОБАВЛЕНО для HTTP сервера
...
def get_prediction(bot: str) -> str:
    ...
    if direction == 'improving':
        if change > 20:
            return "🚀 Бот быстро улучшается! Ожидайте рост скора на 10-15 пунктов на следующей неделе"
        elif change > 10:
...
async def generate_full_report(bot_username: str) -> str:
    ...
    # 12. ПРОГНОЗ
    lines.append("🔮 12. ПРОГНОЗ")
    lines.append(f"   ▫️ {prediction}")
    lines.append("")
    
    # 13. ИТОГОВАЯ ОЦЕНКА
    lines.append("🏆 13. ИТОГОВАЯ ОЦЕНКА")
    ...
    # 15. КРАТКАЯ СВОДКА
    lines.append("📋 15. КРАТКАЯ СВОДКА")
...
# ========== HTTP СЕРВЕР ДЛЯ RENDER (ЧТОБЫ БЫЛ LIVE) ==========
...
async def main():
    ...
    # Запускаем HTTP сервер для Render (чтобы был Live)
