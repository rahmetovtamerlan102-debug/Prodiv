...
async def save_check(botname: str, success: bool, rt: Optional[int], error: str = ""):
    now = int(time.time())
    await db.execute(
        "INSERT INTO checks(bot, ts, success, rt, error) VALUES (?,?,?,?,?)",
        (botname, now, 1 if success else 0, rt, error)
    )
    await db.commit()

# ======================
# РЕАЛЬНАЯ ПРОВЕРКА БОТА (ОТПРАВКА /start)
# ======================
...
async def check_bot(username: str) -> Tuple[bool, int, str]:
    """
    Реальная проверка бота:
    - Получаем entity
    - Отправляем /start
    - Ждём текстовый ответ (таймаут RESPONSE_TIMEOUT)
    - Замеряем время до ответа
    """
    if not await ensure_telethon():
        await save_check(username, False, None, "Telethon не готов")
    ...
    try:
        ...
        if not getattr(entity, 'bot', False):
            await save_check(username, False, None, "Не является ботом")
        ...
        # Ожидаем ответ (текстовое сообщение от бота)
        try:
            response = await client.wait_for(
                lambda m: m.chat_id == entity.id and m.text,
                timeout=RESPONSE_TIMEOUT
            )
            rt = int((time.perf_counter() - start) * 1000)
            await save_check(username, True, rt, "")
            return True, rt, ""
        except asyncio.TimeoutError:
            # Бот не ответил текстом, но сообщение доставлено
            rt = int((time.perf_counter() - start) * 1000)
            await save_check(username, True, rt, "Бот не ответил на /start")
            return True, rt, "Нет ответа, но сообщение отправлено"
        
    except asyncio.TimeoutError:
        await save_check(username, False, None, "Таймаут получения entity")
        return False, 0, "Таймаут получения entity"
    except errors.FloodWaitError as e:
        await asyncio.sleep(min(e.seconds, 30))
        await save_check(username, False, None, f"FloodWait {e.seconds}")
        return False, 0, f"Flood {e.seconds}s"
    except errors.rpcerrorlist.UsernameNotOccupiedError:
        await save_check(username, False, None, "Имя не существует")
        return False, 0, "Имя не существует"
    except Exception as e:
        err_msg = str(e)[:60]
        await save_check(username, False, None, err_msg)
...
async def get_stats(botname: str) -> str:
    ...
    if not rows:
        return f"📊 Статистика @{botname}\n\nНет данных. Сделайте /check @{botname} или /fullreport @{botname}"
...
# ======================
# ФУНКЦИИ ДЛЯ 40-СЕКЦИОННОГО ОТЧЁТА
# ======================

async def get_basic_info(botname: str) -> Dict:
    async with db.execute(
        "SELECT MIN(ts) as first, MAX(ts) as last, COUNT(*) as total, SUM(CASE WHEN success=0 THEN 1 ELSE 0 END) as fails FROM checks WHERE bot=?",
        (botname,)
    ) as cursor:
...
# ======================
# 40 СЕКЦИЙ ОТЧЁТА (БЕЗ СЛЕДОВ МОНИТОРИНГА)
# ======================
...
async def generate_full_report(botname: str) -> str:
    ...
    if len(old)>=5 and len(new)>=5:
        ...
        if old_avg != 0:
            ...
            if imp > 0:
                imp_text = f"Улучшается ({round(abs(imp))}%)"
            elif imp < 0:
                imp_text = f"Ухудшается ({round(abs(imp))}%)"
            else:
                imp_text = "Стабильно"
        else:
            imp_text = "Нет данных"
    ...
    if len(last5) >= 2:
        ...
        if chg < 0:
            lines.append(f"Изменение: {chg:+.1f}% 📈 Улучшается")
        elif chg > 0:
            lines.append(f"Изменение: {chg:+.1f}% 📉 Ухудшается")
        else:
            lines.append("Изменение: 0% Стабильно")
    else:
    ...
    if info['total_checks'] < 20: recs.append("Нужно больше данных для точного анализа")
    ...
    lines.append("Версия бота: 5.0 (аналитический)")
    ...
    lines.append("Алгоритм здоровья: v5")
    
    lines.append("\n📌 [39] РЕЖИМ РАБОТЫ")
    lines.append("Проверка только по командам пользователя")
    lines.append("Автоматические проверки: ОТСУТСТВУЮТ")
    
    lines.append("\n🔚 [40] КОНЕЦ ОТЧЁТА")
    lines.append("Спасибо за использование анализатора ботов")
...
async def cmd_start(m: Message):
    await m.answer(
        "🤖 **Анализатор ботов**\n\n"
        "📌 **Команды:**\n"
        "▪️ /check @bot — быстрая проверка (отправка /start и замер ответа)\n"
...
async def cmd_check(m: Message):
    ...
    await m.answer(f"⏳ Проверка @{botname} (отправка /start)...")
...
async def cmd_fullreport(m: Message):
    ...
    # Делаем проверку (сохраняется в БД)
    ok, rt, err = await check_bot(botname)
    if not ok:
        await m.answer(f"⚠️ Бот @{botname} недоступен. Отчёт будет основан на доступных данных.\nОшибка: {err}")
    # Генерируем отчёт
...
async def main():
    ...
    try:
        ...
        pass
    logger.info("Анализатор ботов готов. Версия 5.0 (без фонового мониторинга)")
