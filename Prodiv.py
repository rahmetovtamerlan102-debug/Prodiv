...
# ========== 2. ДАННЫЕ ОТ @vektokOsint_bot (только если есть номер) ==========
...
# ========== 3. ПОЛУЧЕНИЕ ID ПОЛЬЗОВАТЕЛЯ (userinfobot) ==========
async def get_user_id(username: str) -> Optional[int]:
    resp = await ask_bot("userinfobot", username, timeout=12)
    if resp:
...
# ========== 4. ИСТОРИЯ СМЕНЫ ИМЕНИ/ЮЗЕРНЕЙМА ПО ID (SangMata_beta_bot) ==========
async def get_name_history_by_id(user_id: int) -> List[str]:
    resp = await ask_bot("SangMata_beta_bot", str(user_id), timeout=12)
    history = []
    if resp:
        lines = resp.split('\n')
        for line in lines:
...
# ========== 5. ПРЯМОЙ API (дополнительно ID, имя, био) ==========
...
async def extract_emails_from_bots(username: str, bio: str) -> List[str]:
    emails = []
    for bot in ["liuofxnhvm3dvqbot", "userinfobot"]:
...
# ========== 8. ОСНОВНАЯ ФУНКЦИЯ СБОРА (ПРАВИЛЬНАЯ ЦЕПОЧКА) ==========
async def collect_full_info(username: str) -> Dict[str, Any]:
    ...
    if phone and re.search(r'\d{10,}', phone):
        result["phone"] = phone
        # Шаг 2: данные от vektok (только если есть номер)
        result["vektok"] = await get_vektok_info(phone)
        await asyncio.sleep(BOT_REQUEST_DELAY)
    else:
        result["phone"] = None

    # Шаг 3: получаем числовой ID через userinfobot
    user_id = await get_user_id(username)
    if user_id:
        result["user_id"] = user_id
        # Шаг 4: отправляем ID в SangMata_beta_bot
        result["name_history"] = await get_name_history_by_id(user_id)
        await asyncio.sleep(BOT_REQUEST_DELAY)

    # Прямой API Telegram (дополнительная информация)
    result["direct"] = await get_direct_info(username)

    # Email из ботов и bio
...
def make_report(info: Dict[str, Any]) -> str:
    ...
    # Блок телефона и данных из vektok
    if info.get("phone"):
        lines.append(f"📞 **Номер:** `{info['phone']}`")
        v = info["vektok"]
        if v.get("operator"):
            lines.append(f"📡 **Оператор:** {v['operator']}")
        if v.get("region"):
            lines.append(f"🌍 **Регион:** {v['region']}")
        if v.get("full_name"):
            lines.append(f"👤 **ФИО (базы):** {v['full_name']}")
        if v.get("bases_count"):
            lines.append(f"🗄️ **Базы:** {v['bases_count']} записей")
        if v.get("address"):
            lines.append(f"🏠 **Адрес:** {v['address']}")
    else:
    ...
    # ID (если есть)
    if info["user_id"]:
        lines.append(f"🆔 **ID (userinfobot):** `{info['user_id']}`")

    d = info["direct"]
    if d.get("id"):
        lines.append(f"🆔 **ID (API):** `{d['id']}`")
    if d.get("first_name"):
        lines.append(f"👤 **Имя:** {d['first_name']} {d['last_name'] or ''}")
    if d.get("username"):
        lines.append(f"📛 **Юзернейм:** {d['username']}")
    if d.get("premium"):
        lines.append("⭐ **Telegram Premium**")
    if d.get("reg_date"):
        lines.append(f"📅 **Регистрация (API):** {d['reg_date']}")
    if d.get("status_text"):
        lines.append(f"🕒 **Активность:** {d['status_text']}")
...
async def cmd_start(message: types.Message):
    await message.reply(
        "🔮 *OSINT бот (правильная цепочка)*\n\n"
        "Отправь юзернейм.\n\n"
        "**Цепочка:**\n"
        "1️⃣ @liuofxnhvm3dvqbot → номер телефона\n"
        "2️⃣ @vektokOsint_bot → оператор, регион, ФИО, адрес (только если есть номер)\n"
        "3️⃣ @userinfobot → числовой ID\n"
        "4️⃣ @SangMata_beta_bot → история смены (по ID)\n"
        "5️⃣ Прямой API Telegram → имя, био, общие группы, активность\n"
        "6️⃣ Поиск email и соцсетей\n\n"
...
async def handle_username(message: types.Message):
    ...
    async with lock:
        status_msg = await message.reply("🔎 Сбор данных... до 60 секунд")
...
async def main():
    ...
    # Кэшируем ID всех используемых ботов
    for bot_name in ["liuofxnhvm3dvqbot", "vektokOsint_bot", "userinfobot", "SangMata_beta_bot"]:
