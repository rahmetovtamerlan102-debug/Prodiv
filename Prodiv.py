...
TIMEOUT = 60
MORAX_TIMEOUT = 120               # увеличен для ожидания полноценного ответа
...
# ---------- ОБЫЧНЫЙ ЗАПРОС К БОТУ (БЕЗ ОЖИДАНИЯ ДОЛГИХ ОТВЕТОВ) ----------
...
# ---------- СПЕЦИАЛЬНАЯ ВЕРСИЯ ДЛЯ MORAX (ИГНОРИРУЕТ ПРОМЕЖУТОЧНЫЕ СООБЩЕНИЯ) ----------
async def ask_morax(bot_username: str, query: str, timeout: int = MORAX_TIMEOUT) -> Optional[str]:
    ...
    def handler(event):
        ...
        if not text:
            return
        # Игнорируем служебные / промежуточные сообщения
        if any(word in text.lower() for word in ["вычисляю", "загрузка", "подождите", "идет сбор"]):
            logger.debug(f"Morax: пропущено промежуточное сообщение: {text[:80]}")
            return
        # Принимаем только полноценный ответ (длинный или содержащий ID/usernames)
        if len(text) > 100 or "ID:" in text or "usernames:" in text:
            future.set_result(text)
            user_client.remove_event_handler(handler)
    ...
    try:
        ...
        logger.warning(f"Morax таймаут {timeout} сек, полноценный ответ не получен")
        return None
    finally:
        try:
            user_client.remove_event_handler(handler)
...
# ---------- MORAX (С ИСПОЛЬЗОВАНИЕМ СПЕЦИАЛЬНОЙ ФУНКЦИИ) ----------
async def get_morax_data(username: str) -> Optional[str]:
    # Отправляем юзернейм с одним @ (уже нормализовано)
...
def build_report(data: Dict[str, Any]) -> str:
    ...
    if dates.get("account_age"):
        ...
        clean_age = dates["account_age"].strip('`')
        lines.append(f"📅 Регистрация (оценка): {clean_age}")
...
async def cmd_start(message: types.Message):
    await message.reply(
        ...
        "✅ Morax игнорирует промежуточные сообщения\n"
        "⏳ Ожидание до 2 минут.",
        parse_mode="HTML"
    )
...
async def handle_username(message: types.Message):
    ...
    # Нормализация: убираем все @ в начале, затем добавляем один
    cleaned = raw.lstrip('@')
    if not cleaned:
