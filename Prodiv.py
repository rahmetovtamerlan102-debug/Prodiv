...
# ---------- ГЛОБАЛЬНЫЕ СТРУКТУРЫ С ЗАЩИТОЙ ----------
...
morax_lock = asyncio.Lock()   # для последовательных запросов к Morax (опционально)

# ---------- БЕЗОПАСНАЯ УСТАНОВКА РЕЗУЛЬТАТА ----------
async def safe_set_result(req_id: str, text: str):
    async with pending_lock:
        item = pending_requests.get(req_id)
        if not item:
            return
        bot_id, future, _ = item
        if not future.done():
...
async def global_response_router(event):
    ...
    async with pending_lock:
        if req_id in pending_requests:
            bot_id, future, _ = pending_requests[req_id]
            if bot_id == sender_id and not future.done():
                # безопасный вызов без блокировки цикла
                asyncio.create_task(safe_set_result(req_id, text))

# ---------- ОЧИСТКА ЗАВИСШИХ ЗАПРОСОВ ----------
    ...
    while True:
        ...
        async with pending_lock:
            for req_id in list(pending_requests.keys()):
                _, fut, ts = pending_requests[req_id]
                if now - ts > 120:   # больше 2 минут – отменяем
                    if not fut.done():
                        fut.cancel()
                    pending_requests.pop(req_id, None)

# ---------- УНИВЕРСАЛЬНЫЙ ЗАПРОС К БОТУ ----------
async def ask_bot(bot_username: str, query: str, timeout: int = TIMEOUT, retries: int = RETRY_LIMIT) -> Optional[str]:
    try:
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            logger.warning(f"Таймаут {bot_username} (попытка {attempt+1}/{retries})")
        except Exception as e:
...
async def get_phone(username: str) -> Optional[str]:
    resp = await ask_bot(BOT_LIU, username, timeout=50)
    if resp:
...
async def get_morax_data(username: str) -> Optional[str]:
    async with morax_lock:
        ...
        def handler(event):
            text = event.message.text or event.raw_text or ""
            if text and len(text) > 50 and "сбор" not in text.lower():
                if not future.done():
                    future.set_result(text)
        user_client.add_event_handler(handler, events.NewMessage(from_users=entity.id))
        try:
            await user_client.send_message(entity, f"@{username}")
            result = await asyncio.wait_for(future, timeout=MORAX_TIMEOUT)
            return result
        except asyncio.TimeoutError:
            logger.warning("Morax таймаут")
            return None
        finally:
            user_client.remove_event_handler(handler)

# ---------- ОСНОВНАЯ ФУНКЦИЯ СБОРА (ПАРАЛЛЕЛЬНО) ----------
async def collect_all(username: str) -> Dict[str, Any]:
    ...
    # Запускаем независимые задачи параллельно
    ...
    phone, dates, direct, morax = await asyncio.gather(
        phone_task, dates_task, direct_task, morax_task, return_exceptions=True
    )
    # Обработка возможных исключений (если не None)
    if isinstance(phone, Exception):
        logger.error(f"Ошибка phone: {phone}")
        phone = None
    if isinstance(dates, Exception):
        logger.error(f"Ошибка dates: {dates}")
        dates = {}
    if isinstance(direct, Exception):
        logger.error(f"Ошибка direct: {direct}")
        direct = {}
    if isinstance(morax, Exception):
        logger.error(f"Ошибка morax: {morax}")
        morax = None

    # vektok зависит от номера
    if phone:
...
async def cmd_start(message: types.Message):
    await message.reply(
        "🔮 **OSINT-бот PROD (стабильный)**\n\n"
        "Отправь юзернейм (например, @durov).\n\n"
        "✅ Сбор данных параллельно\n"
        "✅ Нет утечек памяти\n"
        "✅ Автоочистка зависших запросов\n"
        "⏳ Ожидание до 2 минут.",
        parse_mode="HTML"
    )
...
async def main():
    ...
    asyncio.create_task(cleanup_pending())   # запускаем фоновую очистку
