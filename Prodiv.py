...
# ======================
# ПРОВЕРКА БОТА (РЕАЛЬНАЯ ОТПРАВКА /start)
# ======================
...
async def check_bot(username: str) -> Tuple[bool, int, str]:
    ...
    try:
        ...
        await save_check(username, False, None, f"Flood {e.seconds}")
        ...
        err = str(e)[:60]
        await save_check(username, False, None, err)
        return False, 0, err
...
async def generate_full_report(botname: str) -> str:
    ...
    lines.append("Версия: 5.0 (аналитический)")
...
async def cmd_fullreport(m: Message):
    ...
    ok, rt, err = await check_bot(botname)
    if not ok:
        await m.answer(f"⚠️ Бот @{botname} недоступен. Отчёт будет частичным.\nОшибка: {err}")
...
async def main():
    ...
    try:
        ...
        pass
    logger.info("Анализатор ботов готов (версия 5.0, без мониторинга)")
