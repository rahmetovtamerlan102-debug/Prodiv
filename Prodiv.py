...
# Словарь для очередей ожидающих проверок
...
# ======================
# ГЛОБАЛЬНЫЙ ОБРАБОТЧИК СОБЫТИЙ (ОДИН НА ВСЕХ)
# ======================

async def setup_global_handlers():
    """Устанавливает глобальные обработчики событий Telethon"""
    
    @client.on(events.NewMessage)
    async def message_handler(event):
        """Ловим все новые сообщения"""
        sender_id = event.sender_id
        if sender_id and sender_id in pending_checks:
            try:
                await pending_checks[sender_id].put(("message", event))
                logger.info(f"Поймано сообщение от {sender_id}")
            except Exception as e:
                logger.error(f"Ошибка при обработке сообщения: {e}")
    
    @client.on(events.MessageEdited)
    async def edit_handler(event):
        """Ловим редактирование сообщений"""
        sender_id = event.sender_id
        if sender_id and sender_id in pending_checks:
            try:
                await pending_checks[sender_id].put(("edit", event))
                logger.info(f"Поймано редактирование от {sender_id}")
            except Exception as e:
                logger.error(f"Ошибка при обработке редактирования: {e}")
    
    @client.on(events.CallbackQuery)
    async def callback_handler(event):
        """Ловим callback-кнопки"""
        user_id = event.user_id
        if user_id and user_id in pending_checks:
            try:
                await pending_checks[user_id].put(("callback", event))
                logger.info(f"Поймана callback кнопка от {user_id}")
            except Exception as e:
                logger.error(f"Ошибка при обработке callback: {e}")
    
    logger.info("Глобальные обработчики событий установлены")

# ======================
# ПРОВЕРКА БОТА (ЧЕРЕЗ ОЧЕРЕДЬ СОБЫТИЙ)
# ======================
...
async def check_bot(username: str) -> Tuple[bool, int, int, str]:
    ...
    try:
        ...
        # Создаём очередь для этого бота
        queue = asyncio.Queue()
        pending_checks[bot_id] = queue
        
        start = time.perf_counter()
        await client.send_message(entity, "/start")
        logger.info(f"Отправлен /start боту {username} (ID: {bot_id})")
        
        try:
            # Ждём любое событие от бота (сообщение, редактирование, callback)
            event_type, event = await asyncio.wait_for(queue.get(), timeout=RESPONSE_TIMEOUT)
            rt = int((time.perf_counter() - start) * 1000)
            await save_check(username, bot_id, True, rt, "")
            logger.info(f"✅ {username} ответил за {rt} мс (тип: {event_type})")
            ...
            # Удаляем очередь для этого бота
...
async def generate_full_report(botname: str) -> str:
    ...
    lines.append(f"   Обработка ответов: глобальная очередь событий")
    lines.append("")
...
async def main():
    ...
    logger.info("Анализатор ботов готов. Глобальная очередь событий. Таймаут: 8 секунд")
