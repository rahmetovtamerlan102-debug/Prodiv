...
# ======================
# БАЗА ДАННЫХ (единое подключение)
# ======================
...
# ======================
# СНАПШОТ ДЛЯ ОТЧЁТА (ОДИН ПРОХОД ПО БАЗЕ)
# ======================
...
async def get_keyboard(botname: str):
    watched = await is_watched(botname)
    watch_text = "👁️ Убрать" if watched else "👁️ Наблюдать"
    watch_data = f"unwatch:{botname}" if watched else f"watch:{botname}"
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton("🔍 Проверить", callback_data=f"check:{botname}")],
        [InlineKeyboardButton("📊 Краткий отчёт", callback_data=f"quick:{botname}")],
        [InlineKeyboardButton("📋 Полный отчёт (40 секций)", callback_data=f"full:{botname}")],
        [InlineKeyboardButton(watch_text, callback_data=watch_data)]
    ])
...
async def handle_message(m: Message):
    if m.text and m.text.startswith("@"):
        name = m.text[1:].strip()
        await m.answer(f"🤖 Бот @{name}", reply_markup=await get_keyboard(name))
...
async def handle_callback(c: CallbackQuery):
    ...
    if action == "check":
        await c.message.edit_text(f"⏳ Проверка @{name}...")
        ok, rt, err = await check_bot(name)
        if ok:
            text = f"✅ @{name} ДОСТУПЕН\n⏱️ Время ответа: {rt} мс"
        else:
            text = f"❌ @{name} НЕДОСТУПЕН\n📛 Ошибка: {err}"
        await c.message.edit_text(text, reply_markup=await get_keyboard(name))
    
    elif action == "quick":
        snap = await build_snapshot(name)
        status = "🟢 ОНЛАЙН" if snap.perf.get("success_rate", 0) > 95 else "🟡 ДЕГРАДИРУЕТ" if snap.perf.get("success_rate", 0) > 80 else "🔴 ОФЛАЙН"
        text = (
            f"📊 КРАТКИЙ ОТЧЁТ @{name}\n\n"
            f"Статус: {status}\n"
            f"Среднее время: {snap.perf.get('avg_rt', 0)} мс\n"
            f"Успешность: {snap.perf.get('success_rate', 0):.1f}%\n"
            f"Тренд: {snap.trend.get('text', 'Н/Д')}\n"
            f"Здоровье: {snap.health}/100\n"
            f"Проверок: {snap.perf.get('checks', 0)}"
        )
        await c.message.edit_text(text, reply_markup=await get_keyboard(name))
    
    elif action == "full":
        await c.message.edit_text("⏳ Генерация полного отчёта из 40 секций...")
        report = await generate_full_report(name)
        if len(report) > 3800:
            parts = [report[i:i+3800] for i in range(0, len(report), 3800)]
            for i, part in enumerate(parts):
                if i == 0:
                    await c.message.edit_text(part)
                else:
                    await c.message.answer(part)
        else:
            await c.message.edit_text(report)
    
    elif action == "watch":
        await add_watched(name)
        await c.message.edit_text(f"👁️ @{name} добавлен в список наблюдения", reply_markup=await get_keyboard(name))
    
    elif action == "unwatch":
        await remove_watched(name)
        await c.message.edit_text(f"👁️‍🗨️ @{name} удалён из списка наблюдения", reply_markup=await get_keyboard(name))
...
async def main():
    ...
    if not all([API_ID, API_HASH, SESSION_STRING, BOT_TOKEN]):
        ...
        logger.error("Нужны: API_ID, API_HASH, SESSION_STRING, BOT_TOKEN")
        return
