...
TIMEOUT = 20  # увеличен с 15 до 20
DB_FILE = "monitor.db"
...
async def send_and_wait(entity, text: str, timeout=TIMEOUT):
    """Отправка запроса с корректной обработкой ответа"""
    start = time.time()
...
def calculate_ux_index(bot: str) -> int:
    ...
    if not deep:
        return 50
    speed_score = max(0, min(30, 30 - (stats.get('avg_time', 1000)-100)/30)) if stats.get('avg_time') else 15
...
async def full_deep_check(bot_username: str) -> Dict:
    ...
    # ---- Нагрузочный тест 3 запроса (уменьшил с 5) ----
    load_ok = 0
    for i in range(3):
    ...
    # ---- Сохраняем response_details для fingerprint ----
...
async def generate_full_report(bot_username: str) -> str:
    ...
    if avg_time:
        lines.append(f"   • Средняя скорость: {avg_time:.0f} мс")
    else:
    ...
    lines.append(f"   • Стабильность 5 запросов: {deep.get('stability_ok',0)}/5")
    lines.append(f"   • Нагрузка 3/сек: {deep.get('load_5_rate',0)}/3")
    lines.append(f"   • Burst 3 запроса: среднее {deep.get('burst_avg_ms',0)} мс, потерь {deep.get('burst_loss',0)}/3")
    lines.append("")
...
async def cmd_start(msg: Message):
    await msg.reply(
        "🤖 **Монитор ботов v2 (исправленный)**\n\n"
        "Команды:\n"
        "/check @bot – быстрая проверка (2-3 сек)\n"
        "/fullreport @bot – полный отчёт (30-40 сек)\n"
