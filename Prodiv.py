...
# ========== КЭШ ДЛЯ ПОСЛЕДНЕГО BOT_USERNAME ==========
last_bot_cache: Dict[int, str] = {}
...
def get_main_keyboard(bot_username: str):
    """Главная клавиатура с действиями"""
    buttons = [
        ...
        [InlineKeyboardButton(text="🔄 Другая проверка", callback_data="clear")]
    ]
...
async def handle_message(msg: Message):
    ...
    if text == '/start':
        await msg.reply(
            "🤖 **Монитор ботов**\n\n"
            "📌 Просто отправь мне @username бота, и я покажу меню с действиями:\n\n"
            "• 🔍 Быстрая проверка — время ответа и статус\n"
            "• 📄 Полный отчёт — 19 блоков аналитики\n"
            "• ➕ Добавить в мониторинг — проверка раз в час\n"
            "• ➖ Удалить из мониторинга\n"
            "• 📊 Статистика — среднее время и скор\n\n"
...
async def handle_callback(callback: CallbackQuery):
    ...
    if action == "check":
        ...
        await callback.message.edit_text(
            f"✅ @{username} добавлен в мониторинг\nПроверка будет выполняться раз в час\n\nВыберите действие:",
            reply_markup=get_main_keyboard(username)
        )
