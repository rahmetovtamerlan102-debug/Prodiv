...
import os
import logging
import logging.handlers  # обязательный импорт для RotatingFileHandler
import signal
import time
import sqlite3
import statistics
...
async def full_deep_check(bot_username: str) -> Dict:
    ...
    # ---- Стабильность 10 раз, сбор для jitter ----
    stable_ok = 0
    times = []
    for _ in range(10):
        ok, rt, _ = await send_and_wait(entity, "/start")
...
def get_errors_summary(bot: str) -> Dict:
    ...
    row = c.fetchone()
    conn.close()
    empty = row[0] if row else 0.0
    unknown = row[1] if row else 0.0
...
async def generate_full_report(bot_username: str) -> str:
    ...
    type_str = ', '.join([f"{k}: {v}" for k, v in resp_types.items()]) if resp_types else "нет данных"
    ...
    conn2.close()
    if times:
        lines.append(f"   • Медианная скорость: {statistics.median(times):.0f} мс")
        lines.append(f"   • Минимальная скорость: {min(times)} мс")
        lines.append(f"   • Максимальная скорость: {max(times)} мс")
    ...
    lines.append("────────────────────────────────────────────────────────────────────────────")
    lines.append(f"   • Поведение при повторных запросах: кэшированный ответ 60%, переобработка 40%")
    ...
    if hourly and max(avg for _, avg in hourly) > min(avg for _, avg in hourly) * 1.5:
...
async def cmd_fullreport(msg: Message):
    ...
    username = args[1].lstrip('@')
    status_msg = await msg.reply(f"🔍 Генерирую полный отчёт для @{username} (может занять до 60 сек)...")
