...
async def get_stats(botname: str) -> str:
    ...
    status = "✅ ДОСТУПЕН" if success_rate > 95 else "⚠️ НЕСТАБИЛЕН" if success_rate > 80 else "❌ НЕДОСТУПЕН"
    return (
...
async def generate_full_report(botname: str) -> str:
    ...
    lines.append("🤖 1. РЕАЛЬНЫЙ ID БОТА")
    lines.append(f"ID: {real_bot_id if real_bot_id else 'не определён'}")
    lines.append("")

    lines.append("📊 2. СТАТУС ПРОВЕРКИ")
    ...
    lines.append("⏱ 6. ВРЕМЯ РАБОТЫ (UPTIME)")
    ...
    lines.append("📡 13. АКТИВНОСТЬ")
    lines.append(f"Оценка: {activity}/100")
    lines.append("")

    lines.append("📊 14. ДИНАМИКА УЛУЧШЕНИЙ")
    lines.append(imp_text)
    lines.append("")

    lines.append("🏆 15. ГЛОБАЛЬНЫЙ РЕЙТИНГ")
    lines.append("(требуется больше данных)")
    lines.append("")

    lines.append("🤖 16. КЛАССИФИКАЦИЯ БОТА")
    ...
    lines.append("💾 17. МЕТРИКИ БАЗЫ ДАННЫХ")
    lines.append(f"Записей: {info['total_checks']}")
    lines.append(f"Глубина данных: {days} дней")
    lines.append(f"Размер выборки: {perf['checks']}")
    lines.append("")

    lines.append("⏰ 18. ЧАСЫ ПИК")
    lines.append("(требуется больше данных)")
    lines.append("")

    lines.append("📆 19. НЕДЕЛЬНАЯ АКТИВНОСТЬ")
    lines.append("(требуется больше данных)")
    lines.append("")

    lines.append("📈 20. КРАТКОСРОЧНЫЙ ТРЕНД")
    ...
    lines.append("📉 21. ДОЛГОСРОЧНЫЙ ТРЕНД")
    async with db.execute("SELECT rt FROM checks WHERE bot=? AND rt>0 ORDER BY ts ASC LIMIT 10", (botname,)) as cur:
    ...
    lines.append("🎯 22. ЦЕЛЕВЫЕ МЕТРИКИ")
    ...
    lines.append("❤️ 23. КОМПОНЕНТЫ ЗДОРОВЬЯ")
    ...
    lines.append("❌ 24. ДЕТАЛИЗАЦИЯ ОШИБОК")
    ...
    lines.append("📜 25. ИСТОРИЯ ПРОИЗВОДИТЕЛЬНОСТИ")
    ...
    lines.append("⚡ 26. РАСПРЕДЕЛЕНИЕ СКОРОСТИ")
    ...
    lines.append("📊 27. УРОВЕНЬ ОБСЛУЖИВАНИЯ (SLA)")
    lines.append(f"SLA 500мс: {sla500:.1f}%")
    lines.append(f"SLA 1000мс: {sla1000:.1f}%")
    lines.append(f"Оценка SLA: {grade}")
    lines.append("")

    lines.append("🌍 28. СРАВНЕНИЕ С ДРУГИМИ БОТАМИ")
    lines.append("(требуется больше данных)")
    lines.append("")

    lines.append("📊 29. ДОВЕРИТЕЛЬНЫЙ ИНТЕРВАЛ")
    ...
    lines.append("📈 30. ПРОГНОЗ (7 дней)")
    ...
    lines.append("📋 32. СВОДНАЯ ИНФОРМАЦИЯ")
    ...
    lines.append("🏁 33. ИТОГОВАЯ ОЦЕНКА")
    ...
    lines.append("📊 34. БЫСТРАЯ СТАТИСТИКА")
    ...
    lines.append("🎯 35. ВЕРДИКТ О БОТЕ")
    ...
    lines.append("📅 36. МЕТАДАННЫЕ ОТЧЁТА")
    ...
    lines.append("🔧 37. ОТЛАДОЧНАЯ ИНФОРМАЦИЯ")
    ...
    lines.append("📌 38. РЕЖИМ РАБОТЫ")
    lines.append("Проверка только по командам")
    lines.append("Автоматические проверки: ОТСУТСТВУЮТ")
    lines.append("")

    lines.append("✅ 39. ПРОВЕРКА ЧЕРЕЗ /start")
    lines.append("Бот отправляет /start целевому боту")
    lines.append("Замеряет время до получения ответа")
    lines.append("")
...
async def main():
    ...
    try:
        ...
        logger.warning("Signal handlers не поддерживаются на этой платформе")
