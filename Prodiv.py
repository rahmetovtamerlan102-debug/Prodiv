...
# ========== НОВЫЕ ФУНКЦИИ ДЛЯ ДОПОЛНИТЕЛЬНЫХ МЕТРИК ==========

def get_speed_chart(bot: str) -> str:
    """Генерирует текстовый график скорости за последние 24 часа"""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    ...
    times = [datetime.fromtimestamp(r[0]).strftime('%H:%M') for r in data]
    time_line = "   │" + "".join([" " * (len(t)//2) + t[-2:] + " " * (len(t)//2) for t in times])[:50] + "│"
    chart.append(time_line)
    
    return "\n".join(chart)

def get_warning_if_low_score(score: int) -> str:
    """Предупреждение если скор ниже 60"""
    if score < 60:
        return f"   ⚠️ **ВНИМАНИЕ!** Скор бота {score}/100 — ниже нормы (<60). Рекомендуется оптимизация!"
    elif score < 70:
        return f"   🟡 Скор {score}/100 — чуть ниже оптимального (70+). Есть куда расти"
    else:
        return f"   ✅ Скор {score}/100 — отлично (выше 70)"

def get_monitoring_count() -> int:
    """Сколько ботов сейчас в мониторинге"""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def get_first_seen(bot: str) -> str:
    """Когда бот впервые появился в БД"""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def get_last_seen(bot: str) -> str:
    """Когда бот последний раз проверялся"""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def get_improvement_rate(bot: str) -> str:
    ...
    diff = last - first
    if diff > 10:
        return f"🚀 Значительный рост (+{diff} пунктов за {len(hist)} проверок)"
    elif diff > 5:
        return f"📈 Умеренный рост (+{diff} пунктов)"
    elif diff > 0:
        return f"➡️ Небольшой рост (+{diff} пунктов)"
    elif diff < -10:
        return f"⚠️ Сильное падение ({diff} пунктов)! Требуется внимание"
    elif diff < 0:
...
def get_recommendation_priority(bot: str) -> str:
    ...
    score = deep.get('score', 0)
    if score < 50:
        return "🔴 **ВЫСОКИЙ** — бот требует срочной оптимизации"
    elif score < 70:
...
async def generate_full_report(bot_username: str) -> str:
    ...
    # 11. ЛОГИКА И ПАМЯТЬ
    lines.append("🧠 11. ЛОГИКА И ПАМЯТЬ")
    ...
    # 12. СТАТИСТИКА МОНИТОРИНГА
    lines.append("📊 12. СТАТИСТИКА МОНИТОРИНГА")
    ...
    # 13. СРАВНЕНИЕ С РЫНКОМ
    lines.append("🌍 13. СРАВНЕНИЕ С РЫНКОМ")
    ...
    # 14. ЗДОРОВЬЕ БОТА
    lines.append("🩺 14. ЗДОРОВЬЕ БОТА")
    lines.append(f"   ▫️ {health_summary}")
    if deep.get('jitter_ms', 0) > 150:
        lines.append(f"   ▫️ 📡 Нестабильное соединение (высокий джиттер)")
    if deep.get('unknown_rate', 0) > 30:
        lines.append(f"   ▫️ ❓ Бот плохо понимает команды")
    if deep.get('stability_ok', 3) < 2:
        lines.append(f"   ▫️ 🔄 Частые сбои при обработке")
    if deep.get('cold_ms', 0) > 500:
        lines.append(f"   ▫️ ❄️ Долгий запуск (холодный старт)")
    lines.append("")
    
    # 15. ПРОГНОЗ
    lines.append("🔮 15. ПРОГНОЗ")
    ...
    # 17. РЕКОМЕНДАЦИИ
    lines.append("💡 17. РЕКОМЕНДАЦИИ")
    lines.append(f"   ▫️ Приоритет: {priority}")
    rec_count = 0
    if deep.get('cold_ms', 0) > 400:
        lines.append(f"   ▫️ ⚠️ Холодный старт {deep['cold_ms']} мс — оптимизируйте инициализацию")
        rec_count += 1
    if deep.get('stability_ok', 3) < 2:
        lines.append(f"   ▫️ ⚠️ Низкая стабильность — проверьте обработку ошибок")
        rec_count += 1
    ...
    if hourly_data:
        ...
        diff_percent = round((worst[1] - best[1]) / best[1] * 100) if best[1] > 0 else 0
        if diff_percent > 50:
            lines.append(f"   ▫️ ⚠️ Дневная деградация (+{diff_percent}%) — добавьте кэширование")
            rec_count += 1
    if rec_count == 0:
    ...
    # 18. КРАТКАЯ СВОДКА
    lines.append("📋 18. КРАТКАЯ СВОДКА")
    lines.append(f"   ▫️ Состояние: {'🟢 Жив и отвечает' if status != 'critical' else '🔴 Почти не отвечает'}")
    
    if avg_time is not None:
        if avg_time < 300:
            lines.append(f"   ▫️ Скорость: 🚀 Отличная (быстрее 85% ботов)")
        elif avg_time < 600:
    ...
    lines.append(f"   ▫️ UX: {'🌟 Отличный' if deep.get('ux_index',0) >= 80 else '👍 Хороший' if deep.get('ux_index',0) >= 60 else '⚠️ Средный'}")
    lines.append("")
...
async def cmd_add(msg: Message):
    ...
    await msg.reply(f"✅ @{username} добавлен в мониторинг (проверка раз в час)")
...
async def cmd_remove(msg: Message):
    ...
    await msg.reply(f"✅ @{username} удалён из мониторинга")

@dp.message(Command('list'))
async def cmd_list(msg: Message):
    if not watched_bots:
        return await msg.reply("📭 Список отслеживаемых ботов пуст")
