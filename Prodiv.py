...
async def full_deep_check(bot_username: str) -> Dict:
    ...
    # Сохраняем response_details для стиля ответов
...
async def generate_quick_report(bot_username: str) -> str:
    ...
    if avg_time is not None:
        lines.append(f"📊 Среднее за 20 проверок: {avg_time:.0f} мс")
...
async def generate_full_report(bot_username: str) -> str:
    ...
    if desc:
        lines.append(f"│   📝 Описание    │ {desc[:70]}{'...' if len(desc)>70 else ''}")
    lines.append(f"│   ✅ Верифицирован │ {'Да' if info.get('verified') else 'Нет'}")
    lines.append(f"│   ⭐ Премиум      │ {'Да' if info.get('premium') else 'Нет'}")
    lines.append(f"│   ⚠️ Скам/фейк    │ {info.get('scam',0)} / {info.get('fake',0)}")
    lines.append(f"│   🟢 Статус      │ {info.get('status_text')}")
    lines.append(f"│   📅 Возраст     │ {info.get('account_age')}")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    ...
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    lines.append(f"│   🎯 Итоговый статус │ {status.upper()}")
    
    avg_time = stats.get('avg_time')
    if avg_time is not None:
        speed_icon = "🚀" if avg_time < 300 else "👍" if avg_time < 600 else "🐢"
        lines.append(f"│   ⚡ Средняя скорость │ {avg_time:.0f} мс {speed_icon}")
    else:
        lines.append(f"│   ⚡ Средняя скорость │ нет данных")
    ...
    conn.close()
    if times:
        lines.append(f"│   📊 Минимальная     │ {min(times)} мс")
        lines.append(f"│   📊 Максимальная    │ {max(times)} мс")
        lines.append(f"│   📊 Медианная      │ {statistics.median(times):.0f} мс")
    
    lines.append(f"│   📡 Джиттер (σ)    │ {deep.get('jitter_ms', 0)} мс {jitter_icon}")
    lines.append(f"│   {trend_icon} Тренд           │ {trend['direction']} ({trend['change']:+}%)")
    lines.append(f"│   📶 Доступность    │ {uptime}%")
    lines.append(f"│   ⏱️ Нагрузка (час) │ {avg_load} мс")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    
    # 3. ОШИБКИ И ПРОБЛЕМЫ
    lines.append("❌ 3. ОШИБКИ И ПРОБЛЕМЫ")
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    lines.append(f"│   ⏰ Таймаутов         │ {err.get('timeouts', 0)}")
    lines.append(f"│   📭 Пустых ответов    │ {err.get('empty', 0)}%")
    lines.append(f"│   ❓ Неизвестных команд │ {err.get('unknown', 0)}%")
    lines.append(f"│   🔄 Сбойных проверок  │ {100 - uptime:.1f}%")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    
    # 4. ПОЧАСОВАЯ ПРОИЗВОДИТЕЛЬНОСТЬ
    if hourly_data:
        ...
        lines.append("📅 4. ПОЧАСОВАЯ ПРОИЗВОДИТЕЛЬНОСТЬ")
        lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
        lines.append(f"│   🌙 Лучшее время    │ {best[0]:02d}:00 — {best[1]} мс")
        lines.append(f"│   ☀️ Худшее время    │ {worst[0]:02d}:00 — {worst[1]} мс")
        lines.append(f"│   📊 Разброс        │ +{worst[1] - best[1]} мс ({diff_percent}%)")
        if diff_percent > 50:
            lines.append(f"│   ⚠️ Пиковая нагрузка│ {worst[0]:02d}:00-{worst[0]+2:02d}:00")
        lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
        lines.append("")
    
    # 5. ГЛУБОКИЙ АНАЛИЗ
    lines.append("🧪 5. ГЛУБОКИЙ АНАЛИЗ")
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    ...
    if deep.get('cold_ms') and deep.get('warm_ms'):
        ...
        if diff < 0:
            lines.append(f"│   ✅ Оптимизация    │ быстрее на {abs(diff)} мс")
        else:
            lines.append(f"│   ⚠️ Оптимизация    │ медленнее на {diff} мс")
    lines.append(f"│   🎯 Стабильность   │ {deep.get('stability_ok',0)}/3")
    lines.append(f"│   🔄 Нагрузка 2/сек │ {deep.get('load_5_rate',0)}/2")
    lines.append(f"│   ⏱️ TTFB           │ {deep.get('ttfb_ms', 0)} мс")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    
    # 6. СТИЛЬ ОТВЕТОВ
    lines.append("📝 6. СТИЛЬ ОТВЕТОВ")
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    if response_types:
        ...
        for rtype, count in response_types.items():
            ...
            lines.append(f"│   {icon} {rtype:<12} │ {count} ({percent}%)")
    else:
        lines.append(f"│   📄 Нет данных")
    lines.append(f"│   📦 Средний размер │ {avg_size} байт")
    lines.append(f"│   🧠 Сложность     │ {complexity.get('complexity', 'simple')}")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    ...
    if last_checks:
        ...
        lines.append("📊 7. ПОСЛЕДНИЕ ПРОВЕРКИ")
        lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
        for check in last_checks:
            ...
            lines.append(f"│   {time_str}  {icon}  {rt_str}")
        lines.append(f"│   📊 Успешность    │ {success_count}/{len(last_checks)}")
        lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
        lines.append("")
    
    # 8. ДИНАМИКА СКОРА
    if score_history:
        lines.append("📈 8. ДИНАМИКА СКОРА")
        lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
        for ts, score in score_history:
            date_str = datetime.fromtimestamp(ts).strftime('%d.%m %H:%M')
            icon = "🟢" if score >= 70 else "🟡" if score >= 40 else "🔴"
            lines.append(f"│   {date_str}  {icon}  {score}/100")
        if len(score_history) >= 2:
            first_score = score_history[-1][1]
            last_score = score_history[0][1]
            delta = last_score - first_score
            if delta > 0:
                lines.append(f"│   📈 Динамика      │ +{delta} пунктов (рост)")
            elif delta < 0:
                lines.append(f"│   📉 Динамика      │ {delta} пунктов (падение)")
            else:
                lines.append(f"│   ➡️ Динамика      │ стабильно")
        lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
        lines.append("")
    
    # 9. ЛОГИКА И ПАМЯТЬ
    lines.append("🧠 9. ЛОГИКА И ПАМЯТЬ")
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    lines.append(f"│   🧠 Устойчивость   │ {deep.get('robustness', '—')}")
    lines.append(f"│   🖐️ Отпечаток     │ {deep.get('fingerprint', '—')}")
    lines.append(f"│   🎯 UX индекс     │ {deep.get('ux_index', 0)}/100")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    
    # 10. СРАВНЕНИЕ С РЫНКОМ
    score_val = deep.get('score', 0)
    lines.append("🌍 10. СРАВНЕНИЕ С РЫНКОМ")
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    lines.append(f"│   🏆 Ваш бот       │ {score_val}/100")
    lines.append(f"│   📊 Средний бот   │ 65/100")
    lines.append(f"│   🌟 Топ-10%       │ 85+/100")
    
    if score_val >= 85:
        lines.append(f"│   🥇 Позиция       │ в топ-5% ботов")
    elif score_val >= 75:
        lines.append(f"│   🥈 Позиция       │ выше среднего")
    elif score_val >= 60:
        lines.append(f"│   🥉 Позиция       │ средний уровень")
    else:
        lines.append(f"│   ⚠️ Позиция       │ ниже среднего")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    ...
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    lines.append(f"│   🎯 БОТ СКОР      │ {score_val}/100")
    ...
    if score_val >= 85:
        lines.append(f"│   🏆 Уровень       │ TOP (лучше 95% ботов)")
    elif score_val >= 70:
        lines.append(f"│   🌟 Уровень       │ Хороший")
    elif score_val >= 50:
        lines.append(f"│   📊 Уровень       │ Средний")
    else:
        lines.append(f"│   ⚠️ Уровень       │ Низкий")
    
    lines.append(f"│   🔄 Готовность    │ {'Высокая' if deep.get('load_5_rate',0) >= 2 else 'Средняя' if deep.get('load_5_rate',0) >= 1 else 'Низкая'}")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    ...
    if deep.get('cold_ms', 0) > 400:
        lines.append(f"│   ⚠️ Холодный старт {deep['cold_ms']} мс — оптимизируйте инициализацию")
        rec_count += 1
    if deep.get('stability_ok', 3) < 2:
        lines.append(f"│   ⚠️ Низкая стабильность ({deep.get('stability_ok',0)}/3) — проверьте обработку ошибок")
        rec_count += 1
    if deep.get('unknown_rate', 0) > 20:
        lines.append(f"│   ⚠️ Неизвестных команд {deep.get('unknown_rate',0)}% — добавьте fallback")
        rec_count += 1
    if err.get('empty', 0) > 10:
        lines.append(f"│   ⚠️ Пустых ответов {err.get('empty', 0)}% — проверьте логику")
        rec_count += 1
    if deep.get('jitter_ms', 0) > 150:
        lines.append(f"│   ⚠️ Высокий джиттер {deep.get('jitter_ms', 0)} мс — нестабильный сервер")
        rec_count += 1
    if hourly_data:
        best = min(hourly_data, key=lambda x: x[1])
        worst = max(hourly_data, key=lambda x: x[1])
        diff_percent = round((worst[1] - best[1]) / best[1] * 100) if best[1] > 0 else 0
        if diff_percent > 50:
            lines.append(f"│   ⚠️ Дневная деградация (+{diff_percent}%) — добавьте кэширование")
            rec_count += 1
    if rec_count == 0:
        lines.append(f"│   ✅ Отлично! Бот работает стабильно, рекомендаций нет")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    
    # 13. КРАТКАЯ СВОДКА
    lines.append("📋 13. КРАТКАЯ СВОДКА")
    lines.append("┌────────────────────────────────────────────────────────────────────────────────┐")
    lines.append(f"│   🤖 Состояние     │ {'🟢 Жив и отвечает' if status != 'critical' else '🔴 Почти не отвечает'}")
    
    if avg_time is not None:
        if avg_time < 300:
            lines.append(f"│   ⚡ Скорость      │ 🚀 Отличная (быстрее 85% ботов)")
        elif avg_time < 600:
            lines.append(f"│   ⚡ Скорость      │ 👍 Нормальная")
        else:
            lines.append(f"│   ⚡ Скорость      │ 🐢 Медленная")
    else:
        lines.append(f"│   ⚡ Скорость      │ ❓ Нет данных")
    
    lines.append(f"│   📶 Стабильность  │ {'🟢 Высокая' if deep.get('stability_ok',0) >= 2 else '🟡 Средняя' if deep.get('stability_ok',0) >= 1 else '🔴 Низкая'}")
    lines.append(f"│   ✅ Надёжность   │ {'✅ Хорошая' if uptime >= 95 else '⚠️ Средняя' if uptime >= 85 else '❌ Плохая'}")
    lines.append(f"│   🎯 UX           │ {'🌟 Отличный' if deep.get('ux_index',0) >= 80 else '👍 Хороший' if deep.get('ux_index',0) >= 60 else '⚠️ Средний'}")
    lines.append("└────────────────────────────────────────────────────────────────────────────────┘")
    lines.append("")
    ...
    lines.append("════════════════════════════════════════════════════════════════════════════════")
    lines.append(f"📅 {datetime.now().strftime('%d.%m.%Y %H:%M:%S')} | 📊 Всего проверок: {total_checks} | 🕒 Следующее обновление: /fullreport @{bot_username}")
...
async def cmd_fullreport(msg: Message):
    ...
    username = args[1].lstrip('@')
    status_msg = await msg.reply(f"🔍 Анализ @{username} (20-30 сек)...")
