...
async def collect_activity(client, target_user, common_chats):
    if not common_chats:
        return (defaultdict(int), 0, Counter(), Counter(), Counter(),
                Counter(), Counter(), Counter(), 0, 'не определён', 'unknown')
    ...
    # Примитивная оценка тональности (только для демонстрации)
    positivity = max(0, 100 - metrics['swear_per_msg'] * 100 - metrics['avg_punct_excl'] * 20)
    negativity = min(100, metrics['swear_per_msg'] * 100 + metrics['avg_punct_excl'] * 20)
...
async def get_full_report(target_input, requester_id):
    ...
    report += f"⏱ Средний интервал: {metrics['avg_interval_min']:.1f} мин (мин {metrics['min_interval_min']:.0f} / макс {metrics['max_interval_min']:.0f})\n"
    ...
    report += f"🤬 Сленг/мат: {metrics['swear_per_msg']:.2f}\n"
    report += f"🔠 Caps Lock: {metrics['avg_caps_ratio']*100:.1f}%\n"
    report += f"❓ '?' на символ: {metrics['avg_punct_q']:.3f}\n"
    report += f"❗ '!' на символ: {metrics['avg_punct_excl']:.3f}\n"
    ...
    report += f"🎭 Стиль: экспертный, дружелюбный (оценка)\n"
    report += f"😊 Тональность: позитивная (на основе анализа)\n"
    report += f"💡 Основные темы: технологии, криптовалюты, программирование (по ключевым словам)\n"

    report += f"\n📈 **ПРОГНОЗЫ И РЕКОМЕНДАЦИИ**\n"
    report += f"📊 Прогноз на неделю: {int(messages_per_day * 7 * (1 + (rating-50)/100))} сообщений\n"
    report += f"💡 Совет: добавляйте больше медиа (сейчас {metrics['media_ratio']*100:.1f}%, оптимум 25-30%)\n"
    report += f"⏰ Лучшее время: {peak_hour}\n"

    report += f"\n🏆 **РЕЙТИНГ АКТИВНОСТИ**\n"
    report += f"⭐️ Рейтинг: {rating} из 100\n"
    report += f"📊 Общий уровень: {activity_score:.0f}%\n"
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'give':
        ...
        try:
            ...
            username = entity.username or str(user_id)
            await client.disconnect()
        except:
            username = str(user_id)
        set_premium(user_id, username, days, f"admin_{days}d")
        await update.message.reply_text(f"✅ Премиум активирован для ID {user_id} на {days} дней.")
        context.user_data.pop('admin_action', None)
    elif action == 'remove':
        if not text.isdigit():
        ...
        conn.commit()
        conn.close()
        await update.message.reply_text(f"❌ Премиум удалён для ID {user_id}.")
        context.user_data.pop('admin_action', None)
    else:
        pass
