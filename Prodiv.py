...
def get_speed_chart(bot: str) -> str:
    ...
    time_line = "   │" + "".join(t[-2:] + " " for t in times)[:50] + "│"
...
def get_warning_if_low_score(score: int) -> str:
    ...
    if score < 60:
        return f"   🔴 **КРИТИЧЕСКОЕ ПРЕДУПРЕЖДЕНИЕ!** Скор бота {score}/100 — ниже нормы (<60). Требуется срочная оптимизация!"
    elif score < 70:
        return f"   🟡 **ВНИМАНИЕ!** Скор {score}/100 — чуть ниже оптимального (70+). Есть куда расти"
    else:
...
def get_bot_type(bot: str) -> str:
    """Определяет тип бота по ответам"""
