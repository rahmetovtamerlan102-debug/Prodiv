...
async def get_direct_info(target: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Исправлено для новой версии Telethon
...
# ========== 10. ОСНОВНАЯ ФУНКЦИЯ СБОРА (ПРАВИЛЬНАЯ ЦЕПОЧКА) ==========
async def collect_full_info(username: str) -> Dict[str, Any]:
    ...
    if phone and re.search(r'\d{10,}', phone):
        result["phone"] = phone
        # ШАГ 2: данные от vektok (оператор, регион, ФИО, адрес)
        result["vektok"] = await get_vektok_info(phone)
        await asyncio.sleep(BOT_REQUEST_DELAY)
    else:
        result["phone"] = None

    # ШАГ 3: получаем ID через username_to_id_bot
...
def make_report(info: Dict[str, Any]) -> str:
    ...
    if info["reg_date_bot"]:
        lines.append(f"📅 <b>Регистрация (dateregbot):</b> {info['reg_date_bot']}")
...
async def cmd_start(message: types.Message):
    await message.reply(
        ...
        "5️⃣ @dateregbot → дата регистрации\n"
