...
# ========== БАЗОВЫЙ ОТЧЁТ (бесплатно, мгновенно) ==========
...
# ========== ПРЕМИУМ-ОТЧЁТ (полный, с активностью, если есть общие группы) ==========
async def get_full_report(target_input, requester_id):
    ...
    # Если нет общих групп – сразу отдаём профиль с историей, без активности
    if not common_chats:
        report = f"💎 **ПРЕМИУМ ОТЧЁТ (ограниченный – нет общих групп)**\n\n✈️ TELEGRAM · @{user.username or 'нет'}\n\n"
        report += f"🆔 ID: {user.id}\n"
        if usernames:
        ...
        if un_rows:
            ...
            report += "▸ нет данных\n"
        report += "\n💡 **Для получения полной статистики (активность, частота слов, эмодзи) вступите в общие группы с этим пользователем.**"
        await client.disconnect()
        save_cached_report(requester_id, target_key, report)
        return report

    # Есть общие группы – собираем активность
    try:
        ...
        await client.disconnect()
        return "⏱ Превышено время ожидания (15 сек). Попробуйте позже."
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'awaiting_search':
        if search_type == 'basic':
            ...
            msg = await update.message.reply_text("🔍 Получение полного отчёта... (до 15 сек)")
