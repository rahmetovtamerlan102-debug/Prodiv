...
# ========== 3. ПОЛУЧЕНИЕ ID ПО ЮЗЕРНЕЙМУ ==========
async def get_user_id(username: str) -> Optional[int]:
    resp = await ask_bot("username_to_id_bot", username, timeout=12)
    if resp:
...
async def collect_full_info(username: str) -> Dict[str, Any]:
    ...
    # Шаг 2: ID через username_to_id_bot
    ...
    # Шаг 6: Maigret (опционально, если не установлен – пропускаем)
...
async def cmd_start(message: types.Message):
    await message.reply(
        ...
        "7️⃣ Maigret → профили на 500+ сайтах (опционально)\n\n"
...
async def main():
    ...
    for bot_name in ["liuofxnhvm3dvqbot", "vektokOsint_bot", "username_to_id_bot", "SangMata_beta_bot"]:
