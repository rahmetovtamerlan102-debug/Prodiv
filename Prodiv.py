...
TIMEOUT = 10  # уменьшил с 20 до 10 секунд
DB_FILE = "monitor.db"
...
# ========== ОПТИМИЗИРОВАННЫЙ ГЛУБОКИЙ АНАЛИЗ (8-12 запросов) ==========
async def full_deep_check(bot_username: str) -> Dict:
    ...
    # 4. Неизвестные команды (2 запроса)
    unknown = 0
    empty = 0
    for cmd in ['/xyz123', '!@#$']:
    ...
    # 5. Нагрузка 2 запроса (2 запроса)
    load_ok = 0
    for _ in range(2):
        ok, rt, _ = await safe_send_and_wait(entity, "/start", timeout=10)
        if ok:
...
async def generate_full_report(bot_username: str) -> str:
    ...
    if stats.get('avg_time'):
        avg = stats['avg_time']
        lines.append(f"   ▫️ Средняя скорость: {avg:.0f} мс {speed_icon}")
    ...
    lines.append(f"   ▫️ Таймаутов: {err.get('timeouts', 0)}")
    ...
    lines.append(f"   ▫️ Стабильность: {deep.get('stability_ok',0)}/3")
    lines.append(f"   ▫️ Нагрузка: {deep.get('load_5_rate',0)}/2")
    lines.append("")
    ...
    if score_history:
        ...
        for ts, score in score_history:
            ...
            lines.append(f"   ▫️ {date_str} {icon} {score}/100")
        if len(score_history) >= 2:
            delta = score_history[0][1] - score_history[-1][1]
            if delta > 0:
                lines.append(f"   ▫️ 📈 Скор вырос на +{delta}")
        lines.append("")
    
    # 8. Итоговая оценка
    score_val = deep.get('score', 0)
    lines.append("🏆 8. ИТОГОВАЯ ОЦЕНКА")
