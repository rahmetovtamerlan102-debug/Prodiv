...
async def save_fast_check(botname: str, bot_id: int, results: List[dict]):
    """Сохраняет результаты быстрой проверки (5 замеров)"""
    for r in results:
        await db.execute(
            "INSERT INTO checks(bot, bot_id, ts, success, rt, error, command) VALUES (?,?,?,?,?,?,?)",
...
async def check_bot_once(username: str, command: str = "/start") -> Tuple[bool, int, int, str]:
    """Однократная проверка бота. Возвращает (success, bot_id, rt_ms, error)"""
...
# ======================
# БЫСТРАЯ ПРОВЕРКА (5 ЗАМЕРОВ ДЛЯ /report)
# ======================

async def fast_check_bot(username: str) -> Tuple[bool, int, List[dict]]:
    """5 быстрых проверок для отчёта /report"""
    if not await ensure_telethon():
        return False, 0, []
...
async def monitor_single_bot(botname: str):
    """Мониторинг одного бота раз в час"""
...
async def background_monitor():
    """Запускает мониторинг для всех ботов в списке"""
    while not shutdown_event.is_set():
        current_watched = set()
        async with watched_cache_lock:
        ...
        for botname in current_watched:
            if botname not in monitor_tasks or monitor_tasks[botname].done():
                ...
                logger.info(f"🟢 Запущен мониторинг @{botname} (раз в час)")
...
# ======================
# СТАТИСТИЧЕСКИЕ ФУНКЦИИ ДЛЯ ОТЧЁТОВ
# ======================

async def get_monitor_stats(botname: str) -> dict:
    """Собирает статистику из БД для monitor_report"""
    async with db.execute(
...
# ======================
# ГЕНЕРАЦИЯ ОТЧЁТА /report (5 замеров)
# ======================

async def generate_fast_report(botname: str, results: List[dict]) -> str:
    """Генерирует отчёт как в примере (5 замеров)"""
    ...
    if len(successful) == len(results):
        lines.append("39. Вердикт ▸ ✅ бот работает, ответы {}-{} мс".format(min(rts), max(rts)) if rts else "39. Вердикт ▸ бот работает")
    else:
...
# ======================
# ГЕНЕРАЦИЯ ОТЧЁТА /monitor_report (из БД)
# ======================

async def generate_monitor_report(botname: str, stats: dict) -> str:
    """Генерирует отчёт как в примере (из БД, с P95, трендами, uptime)"""
    ...
    lines.append(f"12. Средний интервал ▸ 60 минут")
...
async def cmd_start(m: Message):
    await m.answer(
        "🤖 **Анализатор ботов**\n\n"
        "📌 **Команды:**\n"
        "▪️ /report @bot — быстрый отчёт (5 замеров)\n"
...
async def main():
    ...
    if not all([API_ID, API_HASH, SESSION_STRING, BOT_TOKEN]):
        ...
        print("\n❌ Ошибка: создайте файл .env с переменными:")
        print("API_ID=1234567")
        print("API_HASH=твой_api_hash")
        print("SESSION_STRING=твоя_сессионная_строка")
