...
# ======================
# ВСПОМОГАТЕЛЬНЫЕ СТАТИСТИЧЕСКИЕ ФУНКЦИИ (сокращённо, но все есть)
# ======================
...
# ======================
# 40 СЕКЦИЙ ОТЧЁТА (только аналитика, без инструкций и завершения)
# ======================
...
async def generate_full_report(botname: str) -> str:
    ...
    lines.append(f"   Описание: {risk_desc if risk == 'Низкий' else 'Присутствуют риски сбоев' if risk == 'Средний' else 'Высокие риски, бот часто недоступен'}")
    lines.append("")
...
async def cmd_start(m: Message):
    await m.answer(
        ...
        "▪️ /report @bot — полный отчёт (40 секций, отправляется файлом)\n"
...
async def main():
    ...
    try:
        ...
        pass
    logger.info("Анализатор ботов готов. Отчёты отправляются файлами.")
