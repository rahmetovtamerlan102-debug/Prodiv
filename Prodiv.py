...
# ========== ОЧЕРЕДЬ ДЛЯ ОТВЕТОВ ОТ БОТОВ ==========
class RequestQueue:
    def __init__(self):
        self.pending = {}

    def register(self, bot_id: int, request_id: str, future: asyncio.Future):
        self.pending[(bot_id, request_id)] = future

    def resolve(self, bot_id: int, request_id: str, text: str):
        key = (bot_id, request_id)
        if key in self.pending and not self.pending[key].done():
            self.pending[key].set_result(text)
        self.pending.pop(key, None)

queue = RequestQueue()
bot_ids_cache: Dict[str, int] = {}

# ========== ЕДИНЫЙ ОБРАБОТЧИК ОТВЕТОВ ОТ БОТОВ ==========
@user_client.on(events.NewMessage(incoming=True))
async def global_bot_answer_handler(event):
    ...
    queue.resolve(event.sender_id, req_id, text)
    logger.debug(f"Ответ от бота {event.sender_id}, req_id={req_id}")
...
async def ask_bot(bot_username: str, query: str, timeout: int = 15) -> Optional[str]:
    ...
    while True:
        ...
        queue.register(bot_id, request_id, future)
        try:
            entity = await user_client.get_entity(bot_username)
            ...
            queue.pending.pop((bot_id, request_id), None)
...
async def search_maigret(username: str) -> List[str]:
    """Запускает maigret --json и возвращает список найденных URL"""
    try:
        ...
        return found[:30]  # не больше 30 ссылок, чтобы не забивать отчёт
...
async def collect_full_info(username: str) -> Dict[str, Any]:
    ...
    # Maigret (запускаем в фоне, не увеличиваем общее время ожидания)
...
async def cmd_start(message: types.Message):
    await message.reply(
        "🔮 *OSINT бот (Maigret + SangMata + dateregbot)*\n\n"
...
async def handle_username(message: types.Message):
    ...
    async with lock:
        ...
        if info["direct"].get("username"):
            kb.add(InlineKeyboardButton("Открыть профиль", url=f"https://t.me/{info['direct']['username'].lstrip('@')}"))
