...
TIMEOUT = 60
MORAX_TIMEOUT = 150               # увеличено до 2.5 минут
...
# ---------- MORAX (С ПРОПУСКОМ ПРОМЕЖУТОЧНЫХ СООБЩЕНИЙ) ----------
async def ask_morax(bot_username: str, query: str, timeout: int = MORAX_TIMEOUT) -> Optional[str]:
    ...
    def handler(event):
        ...
        if any(word in text.lower() for word in ["вычисляю", "загрузка", "подождите", "идет сбор"]):
            ...
            return
        # Принимаем ответ, если он содержит ID, usernames, или достаточно длинный
        if len(text) > 50 or "ID:" in text or "usernames:" in text:
            ...
            logger.debug(f"Morax: ответ не принят (короткий/без данных): {text[:100]}")
...
# ---------- ОТЧЁТ (С ОЧИСТКОЙ ДАТ ОТ СКОБОК) ----------
def clean_date(date_str: str) -> str:
    """Удаляет текст в скобках из даты, например '(4 года 6 месяцев)'"""
    if not date_str:
        return date_str
...
async def cmd_start(message: types.Message):
    await message.reply(
        "🔮 **OSINT-бот (финальная исправленная версия)**\n\n"
        "Отправь юзернейм (например, @durov).\n\n"
        "✅ 1 запрос к каждому боту\n"
        "✅ Параллельный сбор (быстрее)\n"
        "✅ Morax ожидает до 2.5 минут\n"
        "✅ Даты без лишних скобок\n"
        "⏳ Ожидание до 2.5 минут.",
        parse_mode="HTML"
    )
