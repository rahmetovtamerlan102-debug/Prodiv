...
# ---------- MORAX (СТАБИЛЬНЫЙ, БЕЗ УТЕЧЕК ХЕНДЛЕРОВ, С ФИЛЬТРАЦИЕЙ) ----------
def is_valid_morax(text: str) -> bool:
    """Проверяет, что ответ Morax – полноценный профиль, а не мусор"""
    if not text:
        return False
    if len(text) < 200:
        return False
    if "сбор" in text.lower() or "загрузка" in text.lower() or "подождите" in text.lower():
        return False
    if "не удалось распознать" in text.lower():
        return False
    return True

async def get_morax_data(username: str) -> Optional[str]:
    """Отправляет юзернейм и ждёт ответ через add_event_handler (без декоратора)"""
    ...
    def handler(event):
        text = event.message.text or event.raw_text or ""
        if text and is_valid_morax(text) and not future.done():
            future.set_result(text)

    user_client.add_event_handler(handler, events.NewMessage(from_users=entity.id))

    try:
        # Отправляем юзернейм с @ для стабильности
        await user_client.send_message(entity, f"@{username}")
        logger.info(f"[MORAX] Отправлен запрос для @{username}")
        result = await asyncio.wait_for(future, timeout=TIMEOUT)
        logger.info(f"[MORAX] Получен ответ: {result[:200] if result else None}")
        return result
    except asyncio.TimeoutError:
        logger.warning(f"[MORAX] Таймаут {TIMEOUT} сек для @{username}")
        return None
    finally:
...
def build_report(username: str, phone: Optional[str], vek: dict, dates: dict, direct: dict, morax: Optional[str]) -> str:
    ...
    if morax:
        ...
        clean_morax = morax.strip()
        clean_morax = re.sub(r'^[ㅤᅠ\s]+$', '', clean_morax, flags=re.MULTILINE)
...
async def cmd_start(message: types.Message):
    await message.reply(
        "🔮 **OSINT-бот (стабильная версия)**\n\n"
        "Отправь юзернейм (например, @durov).\n\n"
        "**Собираю информацию:**\n"
        "• 📞 номер телефона\n"
        "• 📡 данные из утечек (оператор, регион, ФИО, адрес)\n"
