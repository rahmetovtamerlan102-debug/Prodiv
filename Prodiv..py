...
MAX_CHATS = 3
CACHE_HOURS = 6   # кэш полных отчётов на 6 часов
...
# ========== БЫСТРЫЙ ПРОФИЛЬ (без активности) ==========
async def get_fast_profile(target_input):
    ...
    # Быстрый отчёт
    report = f"⚡ **БЫСТРЫЙ ПРОФИЛЬ** (без активности)\n\n"
    ...
    report += f"🌍 Язык интерфейса: {lang if lang else 'не определён'}\n"
    report += f"🕒 Статус: {status_text}\n└ Видимость: {status_type}\n"
    report += f"📅 Регистрация (оценочно): {reg_year} (возраст ≈ {age_days} дн.)\n"
    ...
    report += f"\n💡 Для получения активности и статистики сообщений выберите **Полный отчёт**"
    await client.disconnect()
    return report

# ========== ПОЛНЫЙ ОТЧЁТ (с кэшированием) ==========
async def get_full_report(target_input, force_refresh=False):
    # Проверяем кэш
    if not force_refresh:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT full_report, generated_at FROM report_cache WHERE user_id = ? AND is_full = 1', (str(target_input).lstrip('@').lower(),))
        row = c.fetchone()
        conn.close()
        if row:
            gen_time = datetime.fromisoformat(row[1])
            if datetime.now() - gen_time < timedelta(hours=CACHE_HOURS):
                return row[0] + f"\n\n📌 (данные из кэша от {gen_time.strftime('%Y-%m-%d %H:%M:%S')})"
    # Генерируем полный отчёт
    report = await generate_full_report(target_input)
    if report.startswith("❌") is False:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('INSERT OR REPLACE INTO report_cache (user_id, full_report, generated_at, is_full) VALUES (?, ?, ?, 1)',
                  (str(target_input).lstrip('@').lower(), report, datetime.now().isoformat()))
        conn.commit()
        conn.close()
    return report

async def generate_full_report(target_input):
    ...
    report += f"▸ Сообщений: {metrics['total_messages']}\n"
    ...
    report += f"▸ Всего реакций: {metrics['reaction_count']} (коэфф. {reaction_per_msg:.2f})\n"
    if top_reactions:
        report += f"▸ Топ реакций: " + ", ".join([f"{r}: {c}" for r,c in top_reactions]) + "\n"
    ...
    report += f"▸ Средний интервал между сообщениями: {metrics['avg_interval_min']:.1f} мин (мин {metrics['min_interval_min']:.0f} / макс {metrics['max_interval_min']:.0f})\n"
    ...
    report += f"▸ Знаки '?': {metrics['avg_punct_q']:.3f}, '!': {metrics['avg_punct_excl']:.3f}\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("⚡ Быстрый отчёт (только профиль)", callback_data="fast")],
        [InlineKeyboardButton("📊 Полный отчёт (с активностью)", callback_data="full")],
    ]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "fast":
        ...
        user_states[update.effective_user.id] = "awaiting_fast"
    elif query.data == "full":
        await query.edit_message_text("Введите @username или ID:")
        user_states[update.effective_user.id] = "awaiting_full"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state:
        ...
        return
    if state == "awaiting_fast":
        msg = await update.message.reply_text("🔍 Получение быстрого профиля...")
        result = await get_fast_profile(text)
        await msg.edit_text(result)
    elif state == "awaiting_full":
        msg = await update.message.reply_text("🔍 Сбор полных данных... (до 20 сек)")
        result = await get_full_report(text)
        await msg.edit_text(result)
